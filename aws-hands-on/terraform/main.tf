...
# Cost-optimal encryption: SSE-S3 (AES256). You can switch to KMS later.
...
# -------------------------
# Lambda packaging (Terraform will package code from repo paths)
# NOTE: CodePipeline later updates code with update-function-code; Terraform creates functions.
# -------------------------
...
data "archive_file" "etl_zip" {
    type = "zip"
    source_dir = "${path.module}/../lambda/etl"
...
resource "aws_iam_role_policy" "codebuild_policy" {
    ...
    policy = jsonencode({
        ...
        Statement=[{
            ...
            Effect="Allow",
            Action=[
            "s3:GetObject","s3:PutObject","s3:GetObjectVersion","s3:GetBucketLocation"
            ],
            Resource=[
...
resource "aws_iam_role_policy" "codepipeline_policy" {
    ...
    Statement=[{
        Effect="Allow",
        Action=[
        "s3:GetObject","s3:GetObjectVersion","s3:PutObject","s3:GetBucketLocation"
        ],
        Resource=[
        aws_s3_bucket.artifacts[0].arn,
        "${aws_s3_bucket.artifacts[0].arn}/*"
